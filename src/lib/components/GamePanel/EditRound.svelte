<script lang="ts">
	...
	let selected_players: Player[] = $state(default_values.selected_players || []);
	...
	function handleSubmit() {
		...
		for (const player of players) {
			if (!voting.has(player.id)) {
				if (approved_num > rejected_num) voting.set(player.id, Vote.Reject);
...
<div
	class="flex flex-col divide-y divide-secondary rounded-md border border-secondary bg-primary *:p-2"
>
	<div>
	...
	<div>
		...
		<div
			class="inline-flex divide-x divide-secondary rounded-md border border-secondary text-xl *:px-5"
		>
			<button onclick={() => (num_fails = Math.min(num_fails + 1, selected_players.length))}
				>+</button
			>
			<button onclick={() => (num_fails = Math.max(num_fails - 1, 0))}>-</button>
		</div>
	</div>
	<button class="text-xl font-bold" onclick={() => handleSubmit()}>Save Round</button>
</div>
